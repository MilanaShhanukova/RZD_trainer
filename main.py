...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id, text='Fttftf - Виртуальный тренажер', reply_markup=markup)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query_id == 'start_button':
        ...
        for i, data in enumerate(qna_files):
            keyboard.append([InlineKeyboardButton(data['name'], callback_data=f'qna_{i}')])
        print(keyboard)
        markup = InlineKeyboardMarkup(keyboard)
        await context.bot.send_message(chat_id=update.effective_chat.id, text='Выберите тему, по которой будет проводиться тестирование:', reply_markup=markup)
    if query_id == 'question_button':
        open_chats[update.effective_chat.id] = {}
        open_chats[update.effective_chat.id]['do_question'] = True
        await context.bot.send_message(chat_id=update.effective_chat.id, text='Задавайте ваш вопрос боту')
    if query_id.split('_')[0] == 'qna':
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text='Выберите раздел:', reply_markup=markup)
    if query_id.split('_')[0] == 'qnasub':
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text=f'Вопрос {open_chats[update.effective_chat.id]["question_num"]}: {text}')
...
async def message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'answer' in open_chats[update.effective_chat.id]:
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text=f'Правильный ответ: {open_chats[update.effective_chat.id]["answer"]}\n{metric_string}')
    ...
    if 'do_question' in open_chats[update.effective_chat.id]:
        ...
        text, ctx, llm_prompt, full_llm_answer = respond(input_text, embeddings_raw, embeddings_text, qna)

        await context.bot.send_message(chat_id=update.effective_chat.id, text=f'{full_llm_answer[0]}\nppl: {full_llm_answer[1].numpy()}')
...
if __name__ == '__main__':
    ...
    embeddings_raw, embeddings_text = create_embeddings(small_texts_parts, 'cuda:0')  # create embeddings
