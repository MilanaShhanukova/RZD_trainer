...
def create_generator(question_type):
    qr_generator = QuestionsGenerator(model_path="model-q4_K.gguf")
...
generation_pipe('generated_data.json', 'ПОРЯДОК ОРГАНИЗАЦИИ ДВИЖЕНИЯ ПОЕЗДОВ НА УЧАСТКАХ ОБОРУДОВАННЫХ ЭЛЕКТРОЖЕЗЛОВОЙ СИСТЕМОЙ', "multiple_choice")
